...
const MyStockBox = ({ name, children, onClick, isSelectCategory = false }: MyStockProps) => {
    return (
        <>
            <div
                className='flex justify-between p-[.8rem] rounded-[.8rem] hover:bg-[var(--moneed-white)] cursor-pointer transition-colors`'
                onClick={onClick}
            >
                <div className='flex items-center gap-[.6rem]'>
                    ...
                    <div className='rounded-[.8rem] bg-[var(--moneed-gray-4)] py-[.2rem] px-[.4rem]'>
                        <span className='text-[1.2rem] font-[400] leading-[135%] text-[var(--moneed-gray-9)]'>
                            appl
                        </span>
                    </div>
