    ...
    return (
        <div onClick={onClick} className={`cursor-pointer ${className}`}>
            ...
            <div className='flex gap-[1.2rem] flex-col mt-[1.2rem]'>
                ...
                {title && (
                    <h3 className='text-[1.8rem] text-[var(--moneed-black)] font-semibold leading-[135%] line-clamp-2'>
                        {title}
                    </h3>
                )}
                {content && (
                    <p className='text-[var(--moneed-gray-7)] text-[1.4rem] leading-[142%] line-clamp-2'>{content}</p>
                )}
            </div>
